...
par = abs(dot(n, line(:,4:6), 2)) < tol;   % if the dot product of the normal to the plane and the line is 0, then the plane and edge are parallel
if par == 1
    disp('The edge and the plane are parallel, resulting in either infinite intersections or no intersections.')
    return
end
point(par,:) = NaN;
t(par) = NaN;
